from data_sources import NotionClient, GoogleDocsClient, ConfluenceClient
from models import BedrockClient, OpenAIClient

class ChatStrategyGenerator:
    def __init__(self):
        # Initialize the clients for Google Docs, Notion, Confluence, Bedrock, and OpenAI
        self.google_docs_client = GoogleDocsClient()
        self.notion_client = NotionClient()
        self.confluence_client = ConfluenceClient()  # New Confluence client
...
"""
    ...
    def generate_response_from_notion(self, user_question, model="bedrock"):
        ...
        notion_context = self.notion_client.get_multiple_pages_content()
...
"""
    ...
    def generate_response_from_confluence(self, user_question, model="bedrock"):
        """
        Generates a response using Confluence content as context and sends it to either Bedrock or OpenAI.
        """
        # Fetch the content from Confluence
        confluence_context = self.confluence_client.get_combined_pages_content()
...
User's Question: {user_question}
Context from Confluence: {confluence_context}
...
"""
            ...
            return "No content found in the Confluence pages."
